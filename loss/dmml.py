...
class DMMLLoss(nn.Module):
    ...
    """
    def __init__(self, args, num_support, distance_mode='hard_mining', margin=0.4, gid=None):
        super().__init__()
    ...
    def forward(self, feature, label):
        ...
        if self.distance_mode == 'center_support':
            ...
            for i, support_inds in enumerate(support_inds_list):
                ####################################################
                if self.args.dmml_dist_metric=='euclidean':
                    dist_all = euclidean_dist(query_samples, feature[support_inds])
                elif self.args.dmml_dist_metric=='cosine':
